...
def sender_thread(comm, app_instance):
    ...
    while not stop_event.is_set():
        ...
        # [개선] 2400bps 전송시간 고려: 7바이트 프레임(703) ≈ 29ms → 여유 포함 40ms
        if baud_rate == 2400:
        ...
        if data_to_send:
            try:
                if comm['mode'] == "시리얼 통신" and comm.get('ser'):
                    ...
                    # [핵심 변경] pre-flush 제거. 불필요한 버퍼 폐기/블로킹 방지
                    # ser.reset_output_buffer()  # 삭제

                    # 쓰기 (write_timeout에 의해 과도 블록 방지)
                    ser.write(data_to_send)

                    # [개선] out_waiting이 과도하게 쌓였을 때만 배출 대기
                    # (2400bps + 7B 프레임은 거의 즉시 전송)
...
class AnemometerTesterApp:
    def __init__(self, master):
        self.master = master
        self.master.title("풍향풍속계 통합 테스터 민쑤 VER 2.7 (최종251127)")
