...
def create_nmea_sentence(direction, speed, format_type="WMT52"):
    ...
    if format_type == "Blue Sonic":
        # Blue Sonic 포맷 (예: $WIMWV,0.0,R,000,M,A*4A\r\n)
        body = f"WIMWV,{speed_str},R,{dir_str},M,A"
    else:
        # WMT52 (Standard NMEA) 포맷 (예: $WIMWV,000,R,0.0,M,A*4A\r\n)
...
def create_703_sentence(direction, speed):
    # 속도: float(0.0) -> int(0) * 100 -> 2바이트 (Little Endian)
    speed_scaled = int(speed * 100)
    # 풍향: float(0) -> int(0) * 20 -> 2바이트 (Big Endian)
    direction_scaled = int(direction * 20)

    # 헤더(1B), 길이(1B), 속도(2B, Little Endian)
...
class AnemometerTesterApp:
    def __init__(self, master):
        ...
        # 메인 창 반응형 설정 (추가)
        # ---------------------------
        # 0행(설정/데이터 프레임)이 확장되도록 설정
        self.master.grid_rowconfigure(0, weight=1) 
        # 0열(통신 설정 프레임)과 1열(데이터 설정 프레임)이 확장되도록 설정
    ...
    def create_widgets(self):
        ...
        # sticky="nsew"를 사용하여 할당된 공간을 모두 채우도록 변경
        setup_frame.grid(row=0, column=0, padx=10, pady=10, sticky="nsew") 

        # 프레임 내부에서 1번 열(입력 필드)이 늘어나도록 설정
        ...
        # sticky="ew"를 사용하여 너비에 맞춰 늘어나도록 변경
        ...
        self.connect_button.grid(row=5, column=0, columnspan=3, pady=10, sticky="ew") # sticky="ew" 추가
        self.disconnect_button = ttk.Button(setup_frame, text="전송 종료", command=self.disconnect_stop, state=tk.DISABLED)
        self.disconnect_button.grid(row=6, column=0, columnspan=3, sticky="ew") # sticky="ew" 추가
        ...
        # sticky="ew"를 사용하여 너비에 맞춰 늘어나도록 변경
        self.speed_entry.grid(row=0, column=1, sticky="ew")
        ...
        # sticky="ew"를 사용하여 너비에 맞춰 늘어나도록 변경
        self.direction_entry.grid(row=1, column=1, sticky="ew")

        self.set_button = ttk.Button(control_frame, text="값 설정", command=self.set_data, state=tk.DISABLED)
        self.set_button.grid(row=2, column=0, columnspan=2, pady=5, sticky="ew") # sticky="ew" 추가

        self.pause_button = ttk.Button(control_frame, text="일시정지", command=self.toggle_pause, state=tk.DISABLED)
        self.pause_button.grid(row=3, column=0, columnspan=2, pady=5, sticky="ew") # sticky="ew" 추가

        ttk.Button(control_frame, text="전송 로그 확인", command=self.open_log_window).grid(row=4, column=0, columnspan=2, pady=10, sticky="ew") # sticky="ew" 추가
        ...
        try:
            ...
            if os.name == 'nt':
                os.startfile("devmgmt.msc")
                 ...
                 messagebox.showinfo("정보", "장치 관리자는 Windows 전용 기능입니다.")
        except:
    ...
    def open_log_window(self):
        ...
        # fill=tk.BOTH, expand=True를 사용하여 할당된 공간을 모두 채우도록 설정
    ...
    def set_data(self):
        ...
        try:
            ...
            messagebox.showerror("입력 오류", f"유효하지 않은 값입니다: {e}")
    ...
    def connect_start(self):
        ...
        try:
            ...
            if port == "포트 없음":
                messagebox.showerror("입력 오류", "사용 가능한 COM 포트를 선택하세요.")
                return
            if interval <= 0:
                messagebox.showerror("입력 오류", "전송 간격은 0보다 커야 합니다.")
                return
        except ValueError:
            messagebox.showerror("입력 오류", "설정 값을 확인하세요 (간격은 숫자).")
            return
        ...
        try:
            ...
            self.status_var.set(f"연결됨: {port}, {baud}bps. 전송 시작.")
