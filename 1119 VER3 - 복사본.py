...
class AnemometerTesterApp:
    def __init__(self, master):
        ...
        master.geometry("800x680") # 높이 약간 증가
        ...
        self.sim_status_var = tk.StringVar(value="현재 송신 중: 설정 필요") # [추가] 현재 송신 상태 표시
    ...
    def create_ui(self):
        ...
        ttk.Button(self.serial_frame, text="새로고침", command=self.update_port_list, width=10).grid(row=0, column=2, sticky="w", padx=(0, 10))
        ...
        self.detail_frame = ttk.Frame(op_frame, height=100) # [높이 수정] 내용 추가를 위해 확장
        ...
        # [추가] 현재 시뮬레이션 송신 중인 값을 보여주는 라벨
        ttk.Label(self.write_widgets_frame, textvariable=self.sim_status_var, foreground="red", font=("Malgun Gothic", 10, "bold")).pack(anchor="c", pady=(5, 0))
    ...
    def apply_values(self):
        ...
        try:
            ...
            # 유효성 검사
            if not (0 <= s_input <= 99):
                messagebox.showwarning("범위 초과", "풍속은 0 ~ 99 사이의 값이어야 합니다.")
                return # 적용 취소
            
            if not (0 <= d_input <= 359):
                messagebox.showwarning("범위 초과", "풍향은 0 ~ 359 사이의 값이어야 합니다.")
                return # 적용 취소
            ...
            # UI 업데이트 (로그 + 현재 송신값 라벨)
            self.sim_status_var.set(f"현재 송신 중: 풍속 {s_input} m/s, 풍향 {d_input}°")
