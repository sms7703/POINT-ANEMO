                ...
                if b"$WIMWV" in data:
                    ...
                    if len(data) >= 13:
                        try:
                            ...
                            parse_result = f"수신 데이터: 풍속: {val2:.1f} m/s, 풍향: {val1:.1f}° (R000)"
                        except:
...
class AnemometerTesterApp:
    def __init__(self, master):
        self.master = master
        master.title("풍향풍속계 통합 테스터 MS-251119-VER8")
    ...
    def on_op_mode_change(self):
        ...
        if mode == OP_MODE_WRITE:
            # 쓰기 모드로 진입 시: 수신 데이터를 '대기 중'으로 초기화
            ...
            # 읽기 모드로 진입 시: 송신 데이터를 '대기 중'으로 초기화
