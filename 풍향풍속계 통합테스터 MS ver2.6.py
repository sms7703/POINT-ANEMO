...
# 전역 상태 변수
# ---------------------------
# [변경] 속도 향상을 위해 Lock 없이 접근 가능한 구조로 변경
current_speed = 0.0
current_direction = 0
is_paused = False
...
# GUI 동기화를 위한 Lock (변수 계산에는 안 쓰고, GUI 쪽 안전장치로만 남김)
...
def sender_thread(comm, app_instance):
    ...
    while not stop_event.is_set():
        ...
        # [변경] 2400bps일 때 물리적 전송 시간 고려
        baud_rate = app_instance.baud_var.get()
        if baud_rate == 2400 and interval < 0.1:
            real_interval = 0.1 # 2400bps의 최소 물리적 한계
        else:
            ...
            if comm['mode'] == "랜 통신":
                ...
                if "703" in comm_fmt:
                    if baud_rate != 2400:
                        ...
                        tx_status_text = f"송신(703): 풍속: {current_speed:.1f} m/s  풍향: {current_direction}°"
                        msg_display, data_to_send = create_703_sentence(current_direction, current_speed)
                else:
                    tx_status_text = f"송신(NMEA): 풍속: {current_speed:.1f} m/s  풍향: {current_direction}°"
        ...
        # 3. 데이터 전송 (Lock 제거 및 Flush 적용)
        if data_to_send:
            try:
                if comm['mode'] == "시리얼 통신" and comm.get('ser'):
                    ser = comm['ser']
                    
                    # [핵심 변경 1] Pre-Flush: 이전에 밀린 데이터 삭제 (반응 속도 핵심)
                    ser.reset_output_buffer()
                    
                    # 쓰기
                    ser.write(data_to_send)
                    
                    # [핵심 변경 2] Post-Flush: 즉시 전송 강제
                    ser.flush()
                    
                    # [로그 최적화] 너무 잦은 GUI 업데이트 방지 (0.3초 제한)
        ...
        # [변경] Loop 끝에서 대기 (time.sleep 대신 Event.wait)
        stop_event.wait(real_interval)
...
def receiver_thread(comm, app_instance):
    while not stop_event.is_set():
        ...
        # 쓰기 모드일 때 수신 버퍼 넘침 방지 (계속 비워줌)
        if app_instance.op_mode_var.get() == OP_MODE_WRITE:
            try:
                if comm['mode'] == "시리얼 통신" and comm.get('ser'):
                    if comm['ser'].in_waiting:
        ...
        try:
            ...
            if comm['mode'] == "시리얼 통신" and comm.get('ser'):
                ...
                try:
                    if ser.in_waiting:
                        stop_event.wait(0.02) # 데이터 파편화 방지
            ...
            if data:
                ...
                if parse_result != "해석 불가":
                    current_op_mode = app_instance.op_mode_var.get()
                    if current_op_mode == OP_MODE_READ:
...
# ================================
# GUI Application (기존 전체 유지)
# ================================
class AnemometerTesterApp:
    def __init__(self, master):
        self.master = master
        self.master.title("풍향풍속계 통합 테스터 민쑤 VER 2.7 (최종 Fix)")
    ...
    def apply_values(self):
        ...
        try:
            ...
            # Atomic assignment in Python (thread-safe enough for simulation)
            current_speed = s_input
            current_direction = d_input
                
            self.sim_status_var.set(f"설정값: 풍속 {s_input} m/s, 풍향 {d_input}°")
            self.update_gui_log(f">>> 값 변경: 풍속:{s_input}, 풍향:{d_input}")
    ...
    def toggle_pause(self):
        global is_paused
        is_paused = not is_paused # Boolean toggle is atomic
