...
# 전역 상태 변수
# ---------------------------
# [변경] 속도 향상을 위해 Lock 없이 접근 가능한 구조로 유지
current_speed = 0.0
current_direction = 0
is_paused = False
...
# GUI 동기화를 위한 Lock (GUI 안전장치로만 사용)
...
def sender_thread(comm, app_instance):
    ...
    while not stop_event.is_set():
        ...
        # TEMP UNLOCK: 2400 고정 해제. 대신 물리 전송시간 기반 최소간격 보호
        if baud_rate == 2400:
            real_interval = max(interval, 0.04)   # 7B @2400bps ≈29ms → 여유 40ms
        elif baud_rate in (9600, 19200):
            real_interval = max(interval, 0.01)   # 7B @9600≈7.3ms, @19200≈3.7ms → 10ms 권장
        else:
        ...
        if op_mode == OP_MODE_WRITE:
            if comm['mode'] == "랜 통신":
                ...
                if "703" in comm_fmt:
                    # TEMP UNLOCK: 이전의 '2400bps 전용' 제한 제거
        ...
        if data_to_send:
            try:
                if comm['mode'] == "시리얼 통신" and comm.get('ser'):
                    ...
                    # WRITE+703 에서는 즉시 배출하여 드라이버 지연 방지
                    if (op_mode == OP_MODE_WRITE) and ("703" in comm_fmt):
                        try:
                            ser.flush()
                        except:
                            pass
...
class AnemometerTesterApp:
    def __init__(self, master):
        self.master = master
        self.master.title("풍향풍속계 통합 테스터 민쑤 VER 2.7 (TEMP 703 Unlock)")
        ...
        try:
            if "시리얼 통신" in mode:
                ...
                self.comm['ser'] = serial.Serial(
                    ...
                    write_timeout=0.05,   # 짧은 write timeout
                    rtscts=False,
                    dsrdtr=False,
                    xonxoff=False
                )
