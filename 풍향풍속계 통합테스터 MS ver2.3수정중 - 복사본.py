...
def sender_thread(comm, app_instance):
    ...
    while not stop_event.is_set():
        ...
        msg_display = ""
        tx_status_text = ""
        should_log = True  # 로그 출력 여부 플래그
        
        # [데이터 생성]
        # 1. 쓰기 모드 (Simulator) - 여기서는 내가 쏘는 값이 중요하므로 로그 남김
        ...
        # 2. 읽기 모드 (Monitor) - 폴링 명령어 로그 삭제 요청 반영
            ...
            if "703" in comm_fmt:
                ...
                # 폴링 명령어 생성은 하되 로그에는 찍지 않음 (RX 집중)
                tx_status_text = f"송신 데이터: {polling_cmd} (Polling...)"
                should_log = False  # <--- [핵심 수정] 읽기 모드 송신 로그 끄기
            ...
            try:
                if comm['mode'] == "시리얼 통신" and comm.get('ser'):
                    if comm['ser'].in_waiting:
                        comm['ser'].read(comm['ser'].in_waiting)
                elif comm['mode'] == "랜 통신" and comm.get('sock'):
                    pass 
            except:
                pass
...
class AnemometerTesterApp:
    def __init__(self, master):
        self.master = master
        self.master.title("풍향풍속계 통합 테스터 MS-251119-VER16 (Clean Log)")
